   ...
   ========================================================================== */
	...
	function animateProgressBars() {
		...
		$(".progress-bar .progress-bar-outer:in-viewport").each(function() {
			...
			var $t = $(this);
			$t.css("background-color", $t.attr("data-barColor"));
			if (!$t.hasClass("already-animated")) {
				$t.addClass("already-animated");
				$t.animate({
		...
		if(typeof $.fn.validate != 'undefined'){
			...
			$('#contact-form').validate({
				...
				submitHandler: function() {
                    ...
                    emailjs.send("gmail","website_email",{name: name, email: email, subject: subject, message: message})
                    .then(function(response) {
                        console.log("SUCCESSful. status=%d, text=%s", response.status, response.text);
                        $('#contact-form').clearForm();
						var result = '<div class="alert success"><i class="fa fa-check-circle-o"></i>The message has been sent!</div>';
						$('#contact-form').clearForm();
						$("#formstatus").html(result);
                    }, function(err) {
                        console.log("FAILED. error=", err);
                        var result = '<div class="alert error"><i class="fa fa-times-circle"></i>' + msg + '</div>';
                        $("#formstatus").html(result);
                    });
				}
			});
...
/* ==========================================================================
   ...
   ========================================================================== */
	...
	$(document).ready(function() {			   
		...
		// Init EmailJS
        (function(){
            emailjs.init("user_cKRLxXNLBRDAZzyh1J5Cx");
        })();
		// sticky header
